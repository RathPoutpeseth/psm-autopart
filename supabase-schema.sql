...
-- Order income tracking (added after finances exists, since it references it).
alter table orders add column if not exists income_logged boolean default false;
alter table orders add column if not exists income_finance_id uuid references finances(id) on delete set null;

-- Per-item income tracking (added after finances exists, since it references it).
alter table order_items add column if not exists income_logged boolean default false;
alter table order_items add column if not exists income_finance_id uuid references finances(id) on delete set null;
