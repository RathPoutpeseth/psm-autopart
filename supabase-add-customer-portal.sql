...
--    Add: a customer login can VIEW only their own orders (placing orders is
--    handled by the function in step 9, which is safer than a raw insert policy).
drop policy if exists "customer view own orders" on orders;
create policy "customer view own orders" on orders for select
  using (public.is_staff() or customer_id = public.my_customer_id());

drop policy if exists "customer view own order_items" on order_items;
create policy "customer view own order_items" on order_items for select
  using (
