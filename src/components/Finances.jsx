...
import { DollarSign, Plus, Trash2, TrendingUp, TrendingDown, Receipt as ReceiptIcon, ArrowUp, ArrowDown } from "lucide-react";
...
export default function Finances({ finances, customers, refresh, myEmail, myName }) {
  ...
  const periodFinances = monthFilter ? finances.filter((f) => f.entry_date.slice(0, 7) === monthFilter) : finances;
  const totalIncome = periodFinances.filter((f) => f.type === "income").reduce((s, f) => s + Number(f.amount), 0);
  const totalExpense = periodFinances.filter((f) => f.type === "expense").reduce((s, f) => s + Number(f.amount), 0);
  const visible = finances
    .filter((f) => filterType === "all" || f.type === filterType)
    .filter((f) => !monthFilter || f.entry_date.slice(0, 7) === monthFilter)
    .sort((a, b) => sortAsc ? a.entry_date.localeCompare(b.entry_date) : b.entry_date.localeCompare(a.entry_date));
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 14, marginBottom: 16, flexWrap: "wrap" }}>
        <StatCard icon={TrendingUp} label={monthFilter ? "Income This Month" : "Total Income"} value={money(totalIncome)} tone="good" />
        <StatCard icon={TrendingDown} label={monthFilter ? "Expenses This Month" : "Total Expenses"} value={money(totalExpense)} tone="danger" />
        <StatCard icon={DollarSign} label={monthFilter ? "Net This Month" : "Net"} value={money(totalIncome - totalExpense)} />
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr>
            <th
              ...
              style={{ cursor: "pointer", textAlign: "left", padding: "8px 10px", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.06em", color: COLORS.textMuted, borderBottom: `1px solid ${COLORS.border}`, whiteSpace: "nowrap", userSelect: "none" }}
            >
              <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                Date {sortAsc ? <ArrowUp size={11} /> : <ArrowDown size={11} />}
              </span>
            </th>
            <Th>Type</Th><Th>Category</Th><Th>Customer</Th><Th>Amount</Th><Th>Note</Th><Th></Th>
          </tr></thead>
          <tbody>
            {visible.map((f) => (
              <tr key={f.id}>
