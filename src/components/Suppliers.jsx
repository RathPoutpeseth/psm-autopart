...
import { Truck, Plus, Trash2, Pencil, X, Check, Paperclip, ExternalLink, ArrowUp, ArrowDown } from "lucide-react";
...
export default function Suppliers({ suppliers, supplierInvoices, refresh }) {
  ...
  const [billSortAsc, setBillSortAsc] = useState(false); // false = newest first (default)
  const [billMonthFilter, setBillMonthFilter] = useState(""); // "" = all time
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 14, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        {billMonthFilter && (
          <StatCard
            icon={Truck}
            label={`Billed in ${monthLabel(billMonthFilter)}`}
            value={money(supplierInvoices.filter((b) => b.invoice_date.slice(0, 7) === billMonthFilter).reduce((s, b) => s + Number(b.amount), 0))}
          />
        )}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, flexWrap: "wrap", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <Select value={billMonthFilter} onChange={(e) => setBillMonthFilter(e.target.value)} style={{ width: 180 }}>
      ...
      <Card style={{ overflow: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr>
            <th
              onClick={() => setBillSortAsc(!billSortAsc)}
            ...
            >
              <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                Date {billSortAsc ? <ArrowUp size={11} /> : <ArrowDown size={11} />}
              </span>
            </th>
            <Th>Supplier</Th><Th>Invoice #</Th><Th>Amount</Th><Th>Status</Th><Th>File</Th><Th>Note</Th><Th></Th>
          </tr></thead>
          <tbody>
            {supplierInvoices
              .filter((b) => !billMonthFilter || b.invoice_date.slice(0, 7) === billMonthFilter)
              .sort((a, b) => billSortAsc ? a.invoice_date.localeCompare(b.invoice_date) : b.invoice_date.localeCompare(a.invoice_date))
              .map((b) => (
              <tr key={b.id}>
