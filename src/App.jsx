...
import { Package, ClipboardList, Users, CalendarDays, DollarSign, LayoutDashboard, Wrench, LogOut, Shield, Truck, KeyRound, Tag } from "lucide-react";
...
const ALL_NAV = [
  ...
  { id: "promo-codes", label: "Promo Codes", icon: Tag, roles: ["owner", "manager"] },
...
export default function App() {
  ...
  const refresh = useCallback(async (userId) => {
    const [inv, cust, ord, stf, fin, sup, supInv, promo, prof, custProf] = await Promise.all([
      ...
      supabase.from("promo_codes").select("*").order("created_at", { ascending: false }).then((r) => r).catch(() => ({ data: [] })),
    ...
    <div style={{ background: COLORS.bg, minHeight: "100vh", fontFamily: "Inter, sans-serif", display: "flex", flexDirection: "column" }}>
      <div style={{ borderBottom: `1px solid ${COLORS.border}`, padding: "16px 22px", display: "flex", alignItems: "center", justifyContent: "space-between", background: COLORS.panel }} className="app-header">
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <img src="/psm-logo.jpg" alt="PSM Auto Part" style={{ width: 38, height: 38, borderRadius: 6, objectFit: "cover" }} />
          <div>
      ...
      <div style={{ display: "flex", flex: 1, minHeight: 0 }} className="app-layout">
        ...
        <div style={{ flex: 1, padding: 24, minWidth: 0, overflowX: "auto" }} className="app-content">
          ...
          {tab === "promo-codes" && <PromoCodes promoCodes={promoCodes} refresh={() => refresh()} />}
